...
def home():
  if g.user:
    email = session["email"]
    return render_template("home.html", email=email, login_status=True)
  else:
...
def table_view_request(imo):
  if g.user:
    try:
      ...
      if (len(filtered_arrive_depart_MPA_df) == 0
      ...
      render_html = merge_arrivedepart_VF_df(filtered_arrive_depart_MPA_df,
                                             VF_Single_Vessel_Positions_df)
      if render_html == 1:
      ...
      return (
          render_template(
              "table_view.html",
              msg=
              f"Something went wrong with the data, please ensure IMO Number is valid. Error = {e}",
          ),
          406,
      )
  else:
        ...
        try:
          ...
          if MPA_vessel_data_df.empty and VF_df.empty:
            ...
            if ETA_df.empty:
              ...
              final_df = VF_df
              print(f"app.py: LBO_data_pull(): ETA_df == empty, only VF_df")
            else:
            ...
            # db_vessel_map.py
            final_df = merge_MPA_vessel_data_df_ETA_df(MPA_vessel_data_df,
                                                       ETA_df)
            ...
            # db_vessel_map.py
            merged_df = merged_MPA_VF_ETA_df(MPA_vessel_data_df, VF_df, ETA_df)
            print(f"app.py: LBO_data_pull(): merged_df LBO MAP == {merged_df}")
            final_df = merged_df
        except Exception as e:
          return (
              render_template(
                  "GNSS_request.html",
                  msg=
                  f"Invalid Vessel IMO. Please ensure Vessel IMO is valid. Error = {e}",
              ),
              406,
          )
      ...
      if len(user_lbo_imei) > 1 or len(user_vessel_imo) > 1:
        ...
        try:
          ...
          if display_data[0] == 1:
            ...
            return render_template(display_data[1], )
...
def UCC_upload():
  if g.user:
    ...
    return render_template("UCC_upload.html",
                           email=email,
...
@app.route("/api/vessel_due_to_arrive_db/receive/<email_url>",
           methods=["POST"])
def RECEIVE_Vessel_due_to_arrive(email_url):
  ...
  # ====================== Store VDA into DB =============================
  result = new_vessel_due_to_arrive(row_data_vessel_due_to_arrive, email,
                                    gsheet_cred_path)
  if result == 1:
...
@app.route("/api/vessel_current_position_db/receive/<email_url>",
           methods=["POST"])
